...
/// Structure contenant toutes les métriques de complexité pour un fichier
...
/// Structure contenant la complexité globale du projet
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProjectComplexity {
    pub level: String,          // "Acceptable", "Moyenne", "Élevée", "Très élevée"
    pub notation: String,        // "O(n)", "O(n log n)", "O(n²)", etc.
...
/// Calcule la complexité cyclomatique (McCabe) d'un fichier
/// Compte les structures de contrôle : if, else, for, while, match, etc.
pub fn calculate_cyclomatic_complexity(content: &str) -> u32 {
    ...
    for line in content.lines() {
        ...
        for keyword in &keywords {
            if keyword.len() == 2 { // Opérateurs logiques
...
/// Calcule la complexité NPath (nombre de chemins d'exécution)
...
pub fn calculate_halstead_volume(content: &str) -> f64 {
    ...
    let keywords = [
        ...
        "impl", "trait", "use", "mod", "pub", "const", "static",
    ];
    ...
    let n = n1 + n2;
    let total_n = (total_n1 + total_n2) as f64;
...
/// où V = volume Halstead, G = complexité cyclomatique, LOC = lignes de code
/// Ajusté avec le pourcentage de commentaires
...
/// Analyse la complexité d'un fichier unique
pub fn analyze_file_complexity(path: &Path) -> Result<FileComplexity, String> {
    ...
    let maintainability_index = calculate_maintainability_index(
        halstead_volume,
        cyclomatic,
        loc_pro,
        loc_phy,
...
/// Extensions de fichiers de code à analyser
const CODE_EXTENSIONS: &[&str] = &[
    "rs", "py", "js", "ts", "jsx", "tsx", "java", "c", "cpp", "h", "hpp",
    "cs", "go", "php", "rb", "swift", "kt", "scala", "dart", "svelte",
];
...
/// Détermine le niveau de complexité et la notation Big O
fn determine_complexity_level(
    ...
    let level = if avg_cyclomatic <= 10.0 && avg_maintainability >= 70.0 {
        "Acceptable"
    } else if avg_cyclomatic <= 20.0 && avg_maintainability >= 50.0 {
        "Moyenne"
    } else if avg_cyclomatic <= 40.0 {
        "Élevée"
    } else {
        "Très élevée"
    };
    
    // Notation Big O basée sur la complexité cyclomatique moyenne et LOC
    let notation = if avg_cyclomatic <= 5.0 {
        "O(1)"
    } else if avg_cyclomatic <= 10.0 {
        "O(log n)"
    } else if avg_cyclomatic <= 20.0 {
        "O(n)"
    } else if avg_cyclomatic <= 40.0 {
        "O(n log n)"
    } else if avg_cyclomatic <= 80.0 {
        "O(n²)"
    } else {
        "O(n³)"
    };
    
    (level.to_string(), notation.to_string())
}

/// Calcule la complexité globale du projet
pub fn calculate_project_complexity(project_path: &Path) -> Result<ProjectComplexity, String> {
    ...
    let sum_maintainability: f64 = file_complexities.iter().map(|f| f.maintainability_index).sum();
    ...
    let (level, notation) = determine_complexity_level(avg_cyclomatic, avg_maintainability, total_loc);
...
/// Formate la complexité pour l'affichage : "Complexité : Acceptable • O(n)"
pub fn format_complexity(complexity: &ProjectComplexity) -> String {
    format!("{} • {}", complexity.level, complexity.notation)
}
